...
public class TCWordEditorFragment extends Fragment implements
    ...
    private void checkSubTitleAndViewListForTime() {
        if (mSubtitleList != null)
            for (TXVideoEditConstants.TXSubtitle subtitle : mSubtitleList) {
        ...
        if (mWordInfoList != null) {
            for (TCWordInfo wordInfo : mWordInfoList) {
                if (wordInfo.getStartTime() < mStartTime) {
                    wordInfo.setStartTime(mStartTime);
                }
                if (wordInfo.getEndTime() > mEndTime) {
                    wordInfo.setEndTime(mEndTime);
                }
            }
        }
    }
...
//        changeCurSelectedPos(pos);
    ...
    private void changeCurSelectedPos(int pos) {
        ...
        if (mDuration != 0) {
            mRangeSeekBar.setLeftIndex((mWordInfoList.get(pos).getStartTime() - mStartTime) * 100 / mDuration);
            mRangeSeekBar.setRightIndex((mWordInfoList.get(pos).getEndTime() - mStartTime) * 100 / mDuration);
        }
...
//        stopPlay();
    ...
    public void removeInputFragment() {
        if (mWordInputFragment != null && mWordInputFragment.isAdded()) {
            ...
            getChildFragmentManager().beginTransaction().remove(mWordInputFragment).commit();
        }
    }
